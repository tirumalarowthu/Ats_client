...
const roles = ["VLSI Design Engineer"];
...
// const passoutYears = ["2011", "2012", "2013", "2014", "2015", "2016", "2017", "2018", "2019", "2020", "2021", "2022", "2023", "2024"]
const passoutYears = [];
...
const ApplicantRegister = () => {
    ...
    const [formData, setFormData] = useState({
        ...
        comments: [{
            commentBy: "ATS",
            comment: "The applicant has been registered using the ATS app.",  
            cRound:"New"
        }]
    ...
    const validate = () => {
        ...
        if (formData.isExperienced === "") {
            errors.isExperienced = 'Please confirm whether you have experience or not.'
        }
        ...
        if (validate()) {
            ...
            try {
                ...
                try {
                    ...
                    alert(`You have been successfully registered.A test link will be sent to you shortly.`)
                    await axios.post(`${baseUrl}/add/send/${formData.name}/${formData.role}`)
                    //   alert('Email send to Hiring Manager successfully.')
                    //   navigate("/")
                    navigate("/applicant/register/success")
    ...
    return (
        <div className='container border border-2'>
            ...
            <form style={{ width: "600px", margin: "0px auto", backgroundColor: "powderblue" }} className="p-4 border border-2 rounded" onSubmit={handleSubmit}>
                <div className="form-group">
                    <label>Name :</label>
                    <input type="text" name="name" value={formData.name} onChange={handleChange} className="form-control" placeholder="Please enter your name." />
                    {errors.name && <span className="text-danger">{errors.name}</span>}
                </div>
                <div className="form-group">
                    <label>Email :</label>
                    <input type="email" name="email" value={formData.email} onChange={handleChange} className="form-control" placeholder='Please enter your email.' />
                    {errors.email && <span className="text-danger">{errors.email}</span>}
                </div>
                <div className="form-group">
                    <label>Mobile Number :</label>
                    <input type="number" name="mobile" value={formData.mobile} onChange={handleChange} className="form-control" placeholder='Please enter your 10 digits mobile number.' />
                ...
                <div className="form-group">
                    ...
                    <input type="text" name="collegeName" value={formData.collegeName} onChange={handleChange} className="form-control" placeholder='Please enter your college name.' />
                ...
                <div className="form-group">
                    ...
                    <select name="passout" value={formData.passout} onChange={handleChange} className="form-select">
                        <option value="">---Select passout year---</option>
                        {passoutYears.map(b => <option key={b} value={b}>{b}</option>)}
                    </select>
                ...
                <div className="form-group">
                    ...
                    <input type="url" name="resumeLink" value={formData.resumeLink} onChange={handleChange} className="form-control" placeholder='Ex:https://drive.google.com/file/d/1GWHIc3KDrHM/view?usp=sharing' />
                ...
                )}
                {
                    loading ? <button className="btn btn-info" type="button" disabled>
                        <span className="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span>Please wait... </button>
